...
Or connect your channel with /aniconnect cmd if you are anonymous""",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton(
                    "Auth",
                    url=f"https://t.me/{BOT_NAME.replace('@', '')}/?anistart=auth"
                )]]
            )
        )
...
@anibot.on_message(
    ~filters.private & filters.command(
        ["anisettings", f"anisettings{BOT_NAME}"],
        prefixes=trg
    )
)
@control_user
...
@anibot.on_message(
    filters.command(
        ["ame", f"ame{BOT_NAME}", "activity", f"activity{BOT_NAME}"],
        prefixes=trg
    )
)
@control_user
...
async def logout_cmd(client: Client, message: Message, mdata: dict):
    ...
    if gid == user:
        ...
        await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(
                "Logout",
                url=f"https://t.me/{BOT_NAME.replace('@', '')}/?anistart=logout"
            )]])
        )
    ...
    if "~!" in result and "!~" in result:
        ...
        button.append(
            [
                InlineKeyboardButton(
                    text="View spoiler",
                    url=f"https://t.me/{bot}/?anistart=des_{ctgry}_{qry}_desc"
                )
            ]
        )
    if len(result) > 1000:
        ...
        if spoiler is False:
            ...
            button.append([
                InlineKeyboardButton(
                    text="More Info",
                    url=f"https://t.me/{bot}/?anistart=des_{ctgry}_{qry}_{kek}"
                )
            ])
    add_ = ""
    user = q.pop()
